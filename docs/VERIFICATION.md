...
Additional coverage: SQL check constraints, concurrent customer uniqueness, opposing transfers interleaved with credits, pagination with tied timestamps, UTC timestamp materialization, WAT midnight and canonical hash culture independence.
...
- `dotnet test NovaWallet.API.sln --disable-build-servers -m:1 -v:minimal`: **24 unit tests and 14 SQL Server integration tests passed**, none skipped. The earlier 12-test integration run also passed before the extra constraint/uniqueness checks were added.
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/smoke.ps1`: passed against port 8080, including authenticated create/credit/transfer/replay, exact final balances, statements and OpenAPI JSON.
...
SQL locking protects the financial operations across application instances; it does not replace production authorization or schema-management controls. Tests instantiate independent API hosts in one test process, rather than asserting a multi-machine deployment was exercised. No readiness/liveness endpoint, custom correlation-ID middleware, rate limiter or outbox is claimed as implemented.
